...
get.race.proportions <- function(df.acs){
  df.acs %>% 
    # clean NA vs. NaNs
...
get.race.ranks <- function(df.acs, id_cols = c('GEOID', 'NAME')){
...
get.prv.dif.scores <- function(df.acs, id_cols = c('GEOID', 'NAME')){
  # put it all together
  df.acs.ranks <- get.race.ranks(df.acs, id_cols)
...
get.density <- function(df.geo){
  df.geo %>%
    mutate(AREA_METERS = as.numeric(st_area(df.geo)), # gives in meters^2
           DENSITY = pop.TOTE/(AREA_METERS/2.59e6) # gives ppl per miles^2
           )
}
...
# get crime counts in area --------
get.crime.counts <- function(df.data, area.crime){
  df.data$crime.total <- lengths(st_intersects(df.data, area.crime))
  df.data$crime.per.capita <- 1000 * df.data$crime.total / df.data$pop.TOTE
  df.data
